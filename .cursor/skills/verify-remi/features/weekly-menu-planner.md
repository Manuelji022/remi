...
7. Click that row once. `aria-pressed="true"`. The catalog string is `In fridge`, but `.planner-in-fridge-tag` is `text-transform: uppercase`, so Chrome `innerText` is `IN FRIDGE`. Match case-insensitively. Storage `shoppingChecklist['produceAndFreshHerbs::Cherry tomatoes']` is `{ "checked": true, "inFridge": true }`.
...
- `.planner-in-fridge-tag` is visually `IN FRIDGE`. An exact match on `In fridge` against `innerText` fails even when the toggle worked.
